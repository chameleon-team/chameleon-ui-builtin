...
export default class MiniRuntimeCore {
  ...
  init () {
    if (process.env.media !== "build") {
        invariant(!!this.context, "【@didi/chameleon】runtime context should not undefined")
    }
...
function transformComputed(__cml_data__, context) {
  ...
  origComputedKeys.forEach(key => {
    ...
    if (key in __cml_data__) {
      console.error('【@didi/chameleon ERROR】', `the computed key 【${key}】 is duplicated, please check`)
    }
